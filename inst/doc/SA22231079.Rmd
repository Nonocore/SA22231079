...
## Overview
The answers of the homework in this term.
...
$we can see that \hat{\pi} = \frac{2l}{d\hat{p}}，through \delta method we can get that：$
$$
...
πP = P
$$
Then the distribution $π$ is said to be the smooth distribution of a Markov chain with $P$ as the state transfer matrix.
if :
$$
\forall i,j\in S\qquad π_{i}P_{ij}=π_{j}P_{ji}
$$
Then the state distribution $π$ is said to satisfy the meticulous smoothness condition for Markov chains.
Based on the M-H principle of operation:
(1)when $i\neq j$
$$
P(X_{n+1}=j|X_n=i) = P(X_n=j|X_n=i) \cdot P(u \geq a_{ij}) + P(Y=j|X_n=i) \cdot P(u < a_{ij}) \\= P(Y=j|X_n=i) \cdot P(u < a_{ij}) = q_{ij} \cdot a_{ij}
$$
(2)when $i= j$
$$
P(X_{n+1}=j|X_n=i) = P(X_n=j|X_n=i) \cdot P(u \geq a_{ij}) + P(Y=j|X_n=i) \cdot P(u < a_{ij}) \\= P(u \geq a_{ij}) +  P(Y=j|X_n=i) \cdot P(u < a_j) = 1 - a_{ij} + q_{ij}  = q_{ij}
$$
so，
$$
p_{ij}=a_{ij}q_{ji}
...
## HW07
### Ex 1
$$P(Y=1\mid X_{1},X_{2},X_{3})=\frac{\exp(a+b_{1}X_{1}+b_{2}X_{2}+b_{3}X_{3})}{1+\operatorname{exp}(a+b_{1}X_{1}+b_{2}X_{2}+b_{3}X_{3})},\mathrm{where}X_{1}\sim P(1)
，X_2\sim Exp(1)，X_3\sim B(1,0.5).$$
●Design a function that takes as input values N, b1, b2, b3 and \ f_0, and produces the output a.s
●Call this function, input values are N = 10E6,b1 =0,b2= 1,b3= -1, \ f_0 = 0.1, 0.01, 0.001, 0.0001.
●Plot-$\operatorname{log}f_0$ vs a
...
$$
For each $X_i$, we have the following conditional probabilities:
$$
...
$$
1. Likelihood function for maximised observations

To solve for the MLE of $\lambda$, we need to maximise the likelihood function for the observed data. Taking logarithms and solving for $\lambda$, we can obtain the likelihood equation:
$$
...
where $X_i$ denotes the observed data;

$Z_i$ denotes the corresponding implicit variable indicating whether $X_i$ falls in the given interval $(u_i,v_i)$ (if $X_i$ falls in the interval, $Z_i = 1$; otherwise, $Z_i = 0$);

$\lambda$ denotes the parameters of the model。

E step（Expectation Step）：

Calculate the expectation $\hat{Z}_i$ of the hidden variable $Z_i$, for the $i$th observation $X_i$, with：
$$
\hat{Z}_i = P(Z_i = 1 | X_i, \lambda^{(t)}) = \frac{P(u_i < X_i < v_i | \lambda^{(t)})}{P(X_i > u_i | \lambda^{(t)}) - P(X_i > v_i | \lambda^{(t)})}
$$
where $\lambda^{(t)}$ denotes the $\lambda$ value of the current iteration。

M step（Maximization Step）：

Update the parameter $\lambda$ using the observed data and the expectations of the hidden variables. Specifically, maximise the log-likelihood function for the complete data：
$$
\lambda^{(t+1)} = \arg\max_\lambda \sum_{i=1}^{n} \left( \hat{Z}_i \log f(X_i;\lambda) \right)
$$
where $f(x;\lambda)$ is the probability density function of the exponential distribution

Repeat steps E and M until $\lambda$ converges, stop iteration

3. prove that It is shown that the EM algorithm converges to the MLE of the observed data and has linear speed:
For the EM algorithm to converge, the following two conditions need to be satisfied:
    1. the expectation of the log-likelihood function for complete data (corresponding to the E-step) is a convex function.
    2. In step M, we can find the parameter values that maximise the expectation of the log-likelihood function for complete data.
The log-likelihood function for complete data can be expressed as：
$$
\log L_c(\lambda) = \sum_{i=1}^{n} \log f(X_i;\lambda) = \sum_{i=1}^{n} \left( \hat{Z}_i \log f(X_i;\lambda) \right)
$$
where $\hat{Z}_i$ is the expectation of the hidden variable $Z_i$ computed in the E-step.

Using Jensen's inequality, we know that for convex functions, there are
$$\mathbb{E}[g(X)] \geq g(\mathbb{E}[X])$$
Since the function $g(x) = \log x$ is convex, we have that：

$$
\sum_{i=1}^{n} \left( \hat{Z}i \log f(X_i;\lambda) \right) \geq \log \left( \sum{i=1}^{n} \left( \hat{Z}_i f(X_i;\lambda) \right) \right)
$$
The value of the log-likelihood function for complete data monotonically increases in each iteration. Since the log-likelihood function for complete data is upper bounded, the EM algorithm converges.

In addition, each iteration increases the value of the log-likelihood function for the complete data, so the EM algorithm has a linear rate of convergence.

In summary, the EM algorithm converges to MLE and has a linear convergence rate.

(2)if $(u_i, v_i), i=1,...,n(n=10)$ are obversed as (11, 12), (8, 9), (27, 28), (13, 14), (16, 17), (0, 1), (23, 24), (10, 11), (24, 25), (2, 3)Try to implement the above two algorithms separately to get the numerical solution of the MLE of $\lambda$.
```{r}
data <- matrix(c(11, 12,
                 8, 9,
                 27, 28,
...
### 9.8
The source R code are as follows:
